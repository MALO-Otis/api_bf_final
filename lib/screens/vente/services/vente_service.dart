...
import '../models/commercial_models.dart' as commercial;
...
class VenteService {
  ...
  /// 🎯 RÉCUPÈRE LES ATTRIBUTIONS D'UN COMMERCIAL SPÉCIFIQUE
  /// Récupère les attributions depuis /Gestion Commercial/{site}/attributions/{commercialKey}/historique
  Future<List<commercial.AttributionPartielle>> getAttributionsCommercial({
    required String commercialKey,
    String? site,
  }) async {
    try {
      final siteActuel = site ?? _userSession.site ?? 'Site_Inconnu';

      debugPrint(
          '🔍 [VenteService] Récupération attributions pour $commercialKey sur $siteActuel');
      ...
      if (historiqueSnapshot.docs.isEmpty) {
        debugPrint(
            'ℹ️ [VenteService] Aucune attribution trouvée pour $commercialKey');
        return [];
      }

      debugPrint(
          '✅ [VenteService] ${historiqueSnapshot.docs.length} attributions trouvées pour $commercialKey');

      final attributions = historiqueSnapshot.docs
          .map((doc) {
            try {
              final data = doc.data();
              final attribution = commercial.AttributionPartielle.fromMap(data);
              debugPrint(
                  '📄 Attribution: ${attribution.commercialNom} - ${attribution.numeroLot} - ${attribution.quantiteAttribuee} unités - ${attribution.valeurTotale} FCFA');
              return attribution;
            } catch (e) {
              debugPrint(
                  '❌ [VenteService] Erreur parsing attribution ${doc.id}: $e');
              return null;
            }
          })
          .where((attribution) => attribution != null)
          .cast<commercial.AttributionPartielle>()
          .toList();

      debugPrint(
          '📦 [VenteService] ${attributions.length} attributions valides récupérées pour $commercialKey');
      return attributions;
    } catch (e, stackTrace) {
      debugPrint('❌ [VenteService] Erreur récupération attributions: $e');
      debugPrint('📍 Stack trace: $stackTrace');
      return [];
    }
  }

  /// 👑 RÉCUPÈRE TOUTES LES ATTRIBUTIONS DE TOUS LES COMMERCIAUX (ADMIN SEULEMENT)
  /// Parcourt tous les commerciaux et récupère leurs attributions
  Future<List<commercial.AttributionPartielle>> getAllAttributionsAdmin({
    String? site,
  }) async {
    try {
      final siteActuel = site ?? _userSession.site ?? 'Site_Inconnu';
      ...
      debugPrint(
          '👑 [VenteService] ADMIN - Récupération de TOUTES les attributions sur $siteActuel');

      // Liste des commerciaux connus (tu peux l'étendre selon tes besoins)
      ...
      debugPrint(
          '🔍 [VenteService] Recherche dans ${commerciauxConnus.length} commerciaux: ${commerciauxConnus.join(', ')}');

      final List<commercial.AttributionPartielle> toutesLesAttributions = [];

      // Récupérer les attributions de chaque commercial
      for (final commercialKey in commerciauxConnus) {
        try {
          debugPrint('🔄 Récupération attributions pour: $commercialKey');

          final attributionsCommercial = await getAttributionsCommercial(
            commercialKey: commercialKey,
            site: siteActuel,
          );

          if (attributionsCommercial.isNotEmpty) {
            toutesLesAttributions.addAll(attributionsCommercial);
            debugPrint(
                '✅ $commercialKey: ${attributionsCommercial.length} attributions ajoutées');
          } else {
            debugPrint('ℹ️ $commercialKey: aucune attribution');
          }
        } catch (e) {
          debugPrint('❌ Erreur pour $commercialKey: $e');
        }
      }

      // Trier par date décroissante
      toutesLesAttributions
          .sort((a, b) => b.dateAttribution.compareTo(a.dateAttribution));

      debugPrint(
          '🎯 [VenteService] TOTAL ADMIN: ${toutesLesAttributions.length} attributions récupérées');
      debugPrint('📊 Répartition par commercial:');

      final repartition = <String, int>{};
      for (final attribution in toutesLesAttributions) {
        repartition[attribution.commercialNom] =
            (repartition[attribution.commercialNom] ?? 0) + 1;
      }

      repartition.forEach((nom, count) {
        debugPrint('   - $nom: $count attributions');
      });

      return toutesLesAttributions;
    } catch (e, stackTrace) {
      debugPrint(
          '❌ [VenteService] Erreur récupération TOUTES attributions: $e');
      debugPrint('📍 Stack trace: $stackTrace');
      return [];
    }
  }
