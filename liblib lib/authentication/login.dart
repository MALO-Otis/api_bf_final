...
import 'package:apisavana_gestion/screens/administration/services/user_management_service.dart';
import 'package:apisavana_gestion/utils/firebase_windows_fallback.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  void login() async {
    ...
    if (!FirebaseWindowsFallback.isAvailable) {
      // Mode démo - vérifier les identifiants de démo
      if (_isDemoCredentials()) {
        ...
        setState(() {
          isLoading = false;
          errorMessage =
              'Firebase non disponible. Utilisez : demo@apisavana.com / demo123';
        });
        return;
      }
    }
    ...
    try {
      ...
      // Mise à jour de la dernière connexion (Firestore)
      try {
        // On utilise Get.find si le service est déjà injecté, sinon on le crée.
        final svc = Get.isRegistered<UserManagementService>()
            ? Get.find<UserManagementService>()
            : Get.put(UserManagementService());
        await svc.updateLastLogin(userCred.user!.uid);
      } catch (e) {
        print('⚠️ Impossible de mettre à jour la dernière connexion: $e');
      }
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Utilisez : demo@apisavana.com / demo123',
                                      style: TextStyle(
  ...
  bool _isDemoCredentials() {
    ...
    return (email == 'demo@apisavana.com' || email == 'admin@apisavana.com') &&
        password == 'demo123';
  }

  void _loginDemo() {
    // Simuler une connexion réussie en mode démo
    print('🎭 Connexion en mode démo réussie');
