...
class EspaceCommercialController extends GetxController {
  ...
  final RxList<CaisseCloture> clotures =
      <CaisseCloture>[].obs; // nouvelles clôtures pour caisse
  ...
  void _setupRealtimeListeners() {
    ...
    // Listener pour les clôtures (caissier/admin)
    ...
    _realtimeSubscriptions.add(cloturesStream.listen((snapshot) {
      final cls =
          snapshot.docs.map((d) => CaisseCloture.fromMap(d.data())).toList();
      // Filtrage: si non admin/caisse, ne garder que ses propres éventuelles clôtures
      final isPrivileged = isAdminRole || isCashierRole;
      final filtered = isPrivileged
          ? cls
          : cls.where((c) => c.commercialId == _session.email).toList();
