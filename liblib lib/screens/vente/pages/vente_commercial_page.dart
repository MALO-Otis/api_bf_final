...
class _VenteCommercialPageState extends State<VenteCommercialPage>
  ...
  Widget _buildPrelevementsTab(bool isMobile) {
    return Obx(() {
      ...
      if (attributions.isEmpty) {
        return Column(
          children: [
            // Options de tri même si vide
            _buildSortingOptions(isMobile),
            // Loading indicator si en cours de chargement
            if (_isLoading)
              const Expanded(
                child: Center(
                  child: Column(
        ...
        children: [
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...filtered.map((attribution) =>
                    _buildAttributionCard(attribution, isMobile))
              ],
            ),
          ),
        ],
  ...
  Prelevement _convertAttributionToPrelevement(
    ...
    final produitPreleve = ProduitPreleve(
      ...
      quantitePreleve:
          attribution.quantiteRestante, // Utiliser la quantité restante
      prixUnitaire: attribution.prixUnitaire,
      valeurTotale: attribution.prixUnitaire * attribution.quantiteRestante,
    );
            ...
            children: [
                ...
                child: Column(
                  children: [
                    ...
                    _buildDetailRow('Quantité:',
                        '${attribution.quantiteAttribuee} unités', isMobile),
                    const SizedBox(height: 8),
                    _buildDetailRow(
                        'Prix unitaire:',
                    ...
                    _buildDetailRow(
                      'Valeur totale:',
                      VenteUtils.formatPrix(attribution.valeurTotale),
                      isMobile,
                      isTotal: true,
                    ),
                  ],
              ...
              Row(
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _showVenteDialog(attribution),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.green,
                        foregroundColor: Colors.white,
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _showRestitutionDialog(attribution),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.orange,
                        foregroundColor: Colors.white,
                        padding: EdgeInsets.symmetric(
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () => _showPerteDialog(attribution),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.red,
                        foregroundColor: Colors.white,
                        padding: EdgeInsets.symmetric(
