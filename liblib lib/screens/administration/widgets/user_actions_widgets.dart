...
// Helper global pour obtenir la couleur d'un type d'action (réutilisable panneau + cartes)
Color actionColorFor(UserActionType type) {
  switch (type) {
...
/// Panneau complet avec outils de recherche / tri / filtres autour de l'historique
...
enum _HistorySortField { date, type, admin }
...
class _UserActionsPanelState extends State<UserActionsPanel> {
  ...
  Timer? _debounce;
  String _query = '';
  bool _onlyIrreversible = false;
  bool _ascending = false; // date desc par défaut
  _HistorySortField _sortField = _HistorySortField.date;
  _HistoryRange _range = _HistoryRange.thirtyDays;
  final Set<UserActionType> _selectedTypes = {};
  ...
  void _toggleType(UserActionType type) {
    setState(() {
  ...
  void _resetFilters() {
    setState(() {
      _query = '';
      _searchCtrl.clear();
      _onlyIrreversible = false;
      _ascending = false;
      _sortField = _HistorySortField.date;
      _range = _HistoryRange.thirtyDays;
      _selectedTypes.clear();
    });
  }

  List<UserAction> _applyFilters() {
    ...
    Iterable<UserAction> filtered = widget.actions;
    if (minDate != null) {
      filtered = filtered.where((a) => a.timestamp.isAfter(minDate!));
    }
    if (_onlyIrreversible) {
      filtered = filtered.where((a) => a.type.isIrreversible);
    }
    if (_selectedTypes.isNotEmpty) {
      filtered = filtered.where((a) => _selectedTypes.contains(a.type));
    }
    ...
    list.sort((a, b) {
      int cmp;
      switch (_sortField) {
        case _HistorySortField.date:
          cmp = a.timestamp.compareTo(b.timestamp);
          break;
        case _HistorySortField.type:
          cmp = a.type.index.compareTo(b.type.index);
          break;
        case _HistorySortField.admin:
          cmp = a.adminEmail.compareTo(b.adminEmail);
          break;
      }
  ...
  Widget _buildToolbar(bool isMobile) {
    final chipWrap = Wrap(
      spacing: 4,
      runSpacing: -4,
      children: UserActionType.values.map((t) {
        final selected = _selectedTypes.contains(t);
        return FilterChip(
          label: Text(t.displayName, style: TextStyle(fontSize: 11)),
          selected: selected,
          onSelected: (_) => _toggleType(t),
          visualDensity: VisualDensity.compact,
          selectedColor: actionColorFor(t).withOpacity(0.2),
        );
      }).toList(),
    );
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText: 'Rechercher (description / admin / id)',
                ...
                _buildRangeMenu(),
                const SizedBox(width: 8),
                // Toggle irréversibles
                Tooltip(
                  message: 'Afficher uniquement les actions irréversibles',
                  child: InkWell(
                    onTap: () =>
                        setState(() => _onlyIrreversible = !_onlyIrreversible),
                    borderRadius: BorderRadius.circular(8),
                    child: Container(
  ...
  Widget _buildSortMenu() {
    return PopupMenuButton<_HistorySortField>(
      initialValue: _sortField,
      tooltip: 'Changer le critère de tri',
      onSelected: (v) => setState(() => _sortField = v),
      itemBuilder: (c) => [
        const PopupMenuItem(
            value: _HistorySortField.date, child: Text('Tri: Date')),
        const PopupMenuItem(
            value: _HistorySortField.type, child: Text('Tri: Type')),
        const PopupMenuItem(
            value: _HistorySortField.admin, child: Text('Tri: Admin')),
      ],
      child: _buildMenuButton(
        icon: Icons.sort,
        label: _sortField == _HistorySortField.date
            ? 'Date'
            : _sortField == _HistorySortField.type
                ? 'Type'
                : 'Admin',
      ),
    );
  ...
  Widget _buildRangeMenu() {
    return PopupMenuButton<_HistoryRange>(
      initialValue: _range,
      tooltip: 'Plage temporelle',
      onSelected: (v) => setState(() => _range = v),
      itemBuilder: (c) => const [
        PopupMenuItem(
            value: _HistoryRange.sevenDays, child: Text('7 derniers jours')),
        PopupMenuItem(value: _HistoryRange.thirtyDays, child: Text('30 jours')),
        PopupMenuItem(value: _HistoryRange.all, child: Text('Tout')),
      ],
      child: _buildMenuButton(
  ...
  Widget build(BuildContext context) {
    final processed = _applyFilters();
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        _buildToolbar(widget.isMobile),
        // On réutilise le widget existant pour l'affichage groupé
        UserActionsWidget(
          actions: processed,
...
class UserActionsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final irreversible = actions.where((a) => a.type.isIrreversible).toList()
      ..sort((a, b) => b.timestamp.compareTo(a.timestamp));
    final reversible = actions.where((a) => a.type.isReversible).toList()
      ..sort((a, b) => b.timestamp.compareTo(a.timestamp));

    // Utilisation de LayoutBuilder + SingleChildScrollView pour éviter les conflits
    // Expanded dans un contexte à hauteur non bornée (onglet scrollable parent).
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      _buildStatPill('${actions.length}', 'Total'),
                      _buildStatPill('${irreversible.length}', 'Irréversibles',
                          color: Colors.red),
                      _buildStatPill('${reversible.length}', 'Réversibles',
                          color: Colors.green),
                      _buildStatPill(
                        '${actions.where((a) => a.type == UserActionType.deleted).length}',
                        'Suppressions',
                ...
                // Liste structurée (non scrollable — le scroll est géré par SingleChildScrollView)
                Container(
                  color: Colors.white,
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            if (irreversible.isNotEmpty) ...[
                              _buildSectionHeader('Actions irréversibles',
                      ...
                      if (irreversible.isNotEmpty)
                        Column(
                          children: [
                            ...irreversible.map(_buildActionCard),
                      ...
                      Padding(
                        ...
                        child: _buildSectionHeader('Actions réversibles',
                            Icons.history, Colors.green.shade700),
                      ),
                      const SizedBox(height: 8),
                      if (reversible.isEmpty)
                        Padding(
                          ...
                          child: Center(
                            child: Text(
                              'Aucune action réversible enregistrée',
                              style: TextStyle(
                                color: Colors.grey[500],
                        ...
                        Column(
                          children: [
                            ...reversible.map(_buildActionCard),
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: actionColorFor(action.type).withOpacity(0.1),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
                    action.type.icon,
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          ...
                          fontWeight: FontWeight.bold,
                          color: actionColorFor(action.type),
                        ),
                      ),
                      Text(
  ...
  Widget _buildActionBadges(UserAction action) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        if (action.type.isIrreversible)
  ...
  // Removed obsolete _buildFilterChip method after refactor to grouped lists.
  ...
  // _getActionColor supprimé (remplacé par actionColorFor)
