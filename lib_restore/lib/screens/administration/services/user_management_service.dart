...
class UserManagementService extends GetxService {
  ...
  Future<UserStatistics> getUserStatistics() async {
    ...
    try {
      ...
      final stats = UserStatistics(
        ...
        usersByRole: _groupUsersByRoles(users),
  ...
  Map<String, int> _groupUsersByRoles(List<AppUser> users) {
    final Map<String, int> groups = {};
    for (final user in users) {
      if (user.roles.isEmpty) {
        groups['Sans rôle'] = (groups['Sans rôle'] ?? 0) + 1;
        continue;
      }
  ...
  Future<List<AppUser>> getUsers({UserFilters? filters}) async {
    try {
      ...
      if (filters?.role != null && filters!.role!.isNotEmpty) {
        query = query.where('role', arrayContains: filters.role);
      }
      ...
      if (appliedFilters != null &&
          appliedFilters.sortField != UserSortField.role) {
        query = query.orderBy(
          appliedFilters.sortField.firestoreField,
          descending: !appliedFilters.sortAscending,
        );
      } else {
      ...
      if (filters?.searchTerm != null && filters!.searchTerm!.isNotEmpty) {
        ...
        users = users
            .where((user) =>
                ...
                user.roles
                    .any((role) => role.toLowerCase().contains(searchLower)))
            .toList();
      }
      ...
      if ((appliedFilters?.sortField ?? UserSortField.dateCreation) ==
          UserSortField.role) {
        users.sort((a, b) => a.role.compareTo(b.role));
        if (!(appliedFilters?.sortAscending ?? false)) {
  ...
  Future<bool> createUser({
    ...
    try {
      ...
      final normalizedRoles = extractNormalizedRoles(role);
      final rolesToPersist = normalizedRoles.isNotEmpty
          ? normalizedRoles
          : (role.isNotEmpty ? [role] : <String>[]);
  ...
  Future<bool> updateUser(String userId, Map<String, dynamic> updates) async {
    try {
      ...
      await _logUserAction(
        ...
        newValues: updates,
        user: oldUser.copyWith(
          email: updates['email'] ?? oldUser.email,
          nom: updates['nom'] ?? oldUser.nom,
          prenom: updates['prenom'] ?? oldUser.prenom,
          telephone: updates['telephone'] ?? oldUser.telephone,
          roles: (updates['role'] as List<dynamic>?)
                  ?.map((e) => e.toString())
                  .toList() ??
              oldUser.roles,
          site: updates['site'] ?? oldUser.site,
          isActive: updates['isActive'] ?? oldUser.isActive,
          emailVerified: updates['emailVerified'] ?? oldUser.emailVerified,
        ),
        notifyUser: false,
      );
  ...
  Future<bool> toggleUserStatus(String userId, bool isActive) async {
    try {
      ...
      await _logUserAction(
        ...
        user: user.copyWith(isActive: isActive),
      );
  ...
  Future<bool> changeUserRole(String userId, String newRole) async {
    try {
      ...
      print(
          '👤 Changement de rôle principal pour: ${oldUser.nomComplet} (${oldUser.role} → $newRole)');

      final normalizedRoles = extractNormalizedRoles(newRole);
      final updatedRoles =
          normalizedRoles.isNotEmpty ? normalizedRoles : [newRole];

      // Mettre à jour le rôle dans Firestore
      await _usersCollection.doc(userId).update({'role': updatedRoles});
      ...
      await _logUserAction(
        ...
        description:
            'Rôle principal de ${oldUser.nomComplet} changé de ${oldUser.role} vers $newRole par ${_userSession.email}',
        oldValues: {'role': oldUser.roles},
        newValues: {'role': updatedRoles},
        user: oldUser.copyWith(roles: updatedRoles),
      );
  ...
  Future<bool> addRoleToUser(String userId, String roleToAdd) async {
    try {
      final normalized = extractNormalizedRoles(roleToAdd);
      if (normalized.isEmpty) {
        print('⚠️ Rôle à ajouter vide ou invalide: $roleToAdd');
        return false;
      }
      final role = normalized.first;
      final user = await getUserById(userId);
      if (user == null) {
        print('❌ Utilisateur non trouvé pour ajout de rôle: $userId');
        return false;
      }
      if (user.roles.contains(role)) {
        print('ℹ️ Rôle déjà assigné, aucune action nécessaire');
        return true;
      }
      ...
      await _logUserAction(
        ...
        type: UserActionType.roleChanged,
        description:
            'Ajout du rôle $role à ${user.nomComplet} par ${_userSession.email}',
        oldValues: {'role': user.roles},
        newValues: {
          'role': [...user.roles, role]
        },
        user: user.copyWith(roles: [...user.roles, role]),
      );
      print('✅ Rôle $role ajouté à ${user.nomComplet}');
      return true;
    } catch (e) {
  ...
  Future<bool> removeRoleFromUser(String userId, String roleToRemove) async {
    try {
      final normalized = extractNormalizedRoles(roleToRemove);
      if (normalized.isEmpty) {
        print('⚠️ Rôle à retirer invalide: $roleToRemove');
        return false;
      }
      final role = normalized.first;
      final user = await getUserById(userId);
      if (user == null) {
        print('❌ Utilisateur non trouvé pour retrait de rôle: $userId');
        return false;
      }
      if (!user.roles.contains(role)) {
        print('ℹ️ Rôle $role absent, rien à retirer');
        return true;
      }

      await _usersCollection.doc(userId).update({
        'role': FieldValue.arrayRemove([role]),
      });
      ...
      await _logUserAction(
        ...
        type: UserActionType.roleChanged,
        description:
            'Retrait du rôle $role de ${user.nomComplet} par ${_userSession.email}',
        oldValues: {'role': user.roles},
        newValues: {'role': updatedRoles},
        user: user.copyWith(roles: updatedRoles),
      );
      print('✅ Rôle $role retiré de ${user.nomComplet}');
      return true;
    } catch (e) {
      print('❌ Erreur lors du retrait du rôle: $e');
      return false;
    }
  }
  ...
  Future<bool> changeUserSite(String userId, String newSite) async {
    try {
      ...
      await _logUserAction(
        ...
        user: oldUser.copyWith(site: newSite),
      );
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        await _logUserAction(
          ...
          user: AppUser.fromFirestore(querySnapshot.docs.first),
        );
      }
  ...
  _ActionNotification? _buildNotificationContent({
    ...
    final String adminDisplay = (_userSession.nom?.trim().isNotEmpty ?? false)
        ? _userSession.nom!
        : (_userSession.email ?? 'un administrateur');

    List<String> _extractRoles(dynamic value) {
    ...
    switch (type) {
      case UserActionType.created:
        return _ActionNotification(
          title: 'Bienvenue sur ApiSavana',
          message:
              'Votre compte a été créé par $adminDisplay. Connectez-vous et pensez à vérifier votre adresse email pour activer toutes les fonctionnalités.',
          type: 'account',
          metadata: {
            'roles': user.roles,
            'site': user.site,
      ...
      case UserActionType.activated:
        return _ActionNotification(
          ...
          message:
              'Bonne nouvelle ! $adminDisplay vient d\'activer votre accès. Vous pouvez vous connecter dès maintenant.',
          type: 'account',
          sendEmail: true,
          metadata: {
            'isActive': true,
      ...
      case UserActionType.deactivated:
        return _ActionNotification(
          ...
          message:
              'Votre accès à ApiSavana a été désactivé par $adminDisplay. Contactez-le si vous pensez qu\'il s\'agit d\'une erreur.',
          type: 'security',
          priority: 'high',
          sendEmail: true,
          metadata: {
      ...
      case UserActionType.roleChanged:
        final newRoles = _extractRoles(newValues?['role']).isNotEmpty
            ? _extractRoles(newValues?['role'])
            : user.roles;
        final oldRoles = _extractRoles(oldValues?['role']);
        return _ActionNotification(
          title: 'Mise à jour de votre rôle',
          message:
              '$adminDisplay vient de mettre à jour votre rôle: ${newRoles.join(', ')}.${oldRoles.isNotEmpty ? ' (Ancien rôle: ${oldRoles.join(', ')})' : ''}',
          type: 'role',
          sendEmail: true,
          metadata: {
            'oldRole': oldRoles,
      ...
      case UserActionType.siteChanged:
        final previousSite = _stringify(oldValues?['site']);
        final newSite = _stringify(newValues?['site'] ?? user.site);
        return _ActionNotification(
          title: 'Changement de site',
          message:
              '$adminDisplay a changé votre site d\'affectation ${previousSite.isNotEmpty ? 'de $previousSite ' : ''}vers $newSite.',
          type: 'site',
          sendEmail: true,
          metadata: {
            'oldSite': previousSite,
            'newSite': newSite,
          },
        );
      case UserActionType.passwordReset:
        return _ActionNotification(
          ...
          message:
              '$adminDisplay a demandé la réinitialisation de votre mot de passe. Consultez l\'email automatique que vous avez reçu pour terminer l\'opération.',
          type: 'security',
          priority: 'high',
          sendEmail: true,
        );
      case UserActionType.emailVerified:
        return _ActionNotification(
          title: 'Email vérifié',
          message:
              'Votre adresse ${user.email} a été marquée comme vérifiée par $adminDisplay. Vous avez maintenant accès à toutes les fonctionnalités.',
          type: 'account',
        );
      case UserActionType.emailResent:
        return _ActionNotification(
          ...
          message:
              '$adminDisplay vient de renvoyer un email de vérification à ${user.email}. Vérifiez votre boîte de réception (et vos spams).',
          type: 'account',
          metadata: {
            'email': user.email,
          },
      ...
      case UserActionType.passwordGenerated:
        return _ActionNotification(
          ...
          message:
              'Un mot de passe temporaire a été généré par $adminDisplay. Il vous sera communiqué par un canal sécurisé.',
          type: 'security',
          priority: 'high',
        );
      case UserActionType.accessGranted:
        return _ActionNotification(
          title: 'Accès accordé',
          message:
              '$adminDisplay vient d\'accorder un accès spécifique sur votre compte. Vous pouvez désormais utiliser les fonctionnalités associées.',
          type: 'security',
          sendEmail: true,
          metadata: {
            'hasAccess': true,
      ...
      case UserActionType.accessRevoked:
        return _ActionNotification(
          title: 'Accès révoqué',
          message:
              'Votre accès a été révoqué par $adminDisplay. Contactez l\'administration si vous avez besoin d\'éclaircissements.',
          type: 'security',
          priority: 'high',
          sendEmail: true,
          metadata: {
  ...
  Future<void> _logUserAction({
    ...
    if (!notifyUser || type == UserActionType.other) {
      return;
    }
    ...
    try {
      ...
      final notification = _buildNotificationContent(
        type: type,
        user: targetUser,
        description: description,
        oldValues: oldValues,
      ...
      final metadata = <String, dynamic>{
        'actionType': type.toString().split('.').last,
        'loggedBy': _userSession.email,
        'loggedById': _userSession.uid,
        'loggedAt': DateTime.now().toIso8601String(),
        if (notification.metadata.isNotEmpty) ...notification.metadata,
      };
      ...
      await _notificationService.createNotification(
        ...
        userName: targetUser.nomComplet,
      );
    } catch (e) {
      print('Erreur lors de l\'envoi de la notification utilisateur: $e');
    }
  }
