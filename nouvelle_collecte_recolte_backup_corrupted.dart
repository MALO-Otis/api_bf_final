...
class _NouvelleCollecteRecoltePageState
  ...
  // Simple geolocation function - NO UTF-8 PROBLEMS
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          Get.snackbar('Permission refusee', 'Acces geolocalisation requis');
          return;
        }
      }

      // Target < 10m precision
      const double TARGET = 10.0;
      const int MAX_ATTEMPTS = 5;
      Position? bestPosition;
      
      for (int i = 1; i <= MAX_ATTEMPTS; i++) {
        print('RECOLTE attempt $i/$MAX_ATTEMPTS for <${TARGET}m');
        ...
        print('Attempt $i: ${pos.accuracy.toStringAsFixed(1)}m');
        
        if (pos.accuracy < TARGET) {
          Get.snackbar('SUCCESS', 'Precision: ${pos.accuracy.toStringAsFixed(1)}m < ${TARGET}m');
          break;
        }
        
        if (i < MAX_ATTEMPTS) await Future.delayed(Duration(seconds: 3));
      }
      ...
      if (bestPosition != null) {
        setState(() {
          _geolocationData = {
            ...
            'address': 'Lat: ${bestPosition.latitude.toStringAsFixed(6)}, Lng: ${bestPosition.longitude.toStringAsFixed(6)}',
          };
        });
        
        print('RECOLTE FINAL: ${bestPosition.accuracy.toStringAsFixed(1)}m');
        Get.snackbar('Position obtenue', 'Precision: ${bestPosition.accuracy.toStringAsFixed(1)}m');
      }
      
    } catch (e) {
      print('RECOLTE error: $e');
      Get.snackbar('Erreur', 'Impossible obtenir position: $e');
    } finally {
